...
bool BMP::WriteToFile( const char* FileName )
{
 ...
 if( BitDepth == 16 )
 {
  ...
  TempWORD = (ebmpWORD) ((RedWORD<<11) + (GreenWORD<<5) + BlueWORD);
	if( IsBigEndian() )
...
bool BMP::ReadFromFile( const char* FileName )
{
 ...
 if( BitDepth == 16 )
 {
  ...
  { TempShiftWORD = (ebmpWORD)(TempShiftWORD>>1); GreenShift++; }
  int BlueShift = 0;
  TempShiftWORD = BlueMask;
  while( TempShiftWORD > 31 )
  { TempShiftWORD = (ebmpWORD)(TempShiftWORD>>1); BlueShift++; }
  int RedShift = 0;
  TempShiftWORD = RedMask;
  while( TempShiftWORD > 31 )
  { TempShiftWORD = (ebmpWORD)(TempShiftWORD>>1); RedShift++; }
